...
class MY_Parser extends CI_Parser
{
	...
	public function setVar( $data, $value = FALSE ) {
		...
		if ( !empty( $data ) && is_array( $data ) ) {
			if ( !empty( $aVars ) ) {
				$data = array_merge( (array)$aVars, $data );
			}
		} // If passed data is a string and has a value
		elseif ( !empty( $data ) && is_string( $data ) && isset( $value ) ) {
			if ( !empty( $aVars ) ) {
				$data = array_merge( (array)$aVars, array( $data => $value ) );
			}

		} elseif ( !empty( $data ) && is_object( $data ) ) {
			if ( !empty( $aVars ) ) {
				$data = (object)array_merge( (array)$aVars, (array)$data );
			}
		}
		$this->CI->smarty->assign( $sClass, $data );
	}
